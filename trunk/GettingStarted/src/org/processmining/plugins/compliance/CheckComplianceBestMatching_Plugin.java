...
import org.processmining.plugins.compliance.align.PNLogMatchInstancesReplayer;
import org.processmining.plugins.compliance.align.PNLogMatchInstancesReplayer.ReplayParams;
 ...
 * 
 */
@Plugin(name = "Check Compliance Using Conformance Checking (All Best Matching)",
	returnLabels = { "Compliance Diagnostics", "Abstracted Log" },
	returnTypes = { PNMatchInstancesRepResult.class, XLog.class },
...
public class CheckComplianceBestMatching_Plugin {
	...
	@UITopiaVariant(
			...
			pack = "Compliance")
	@PluginVariant(variantLabel = "Check Compliance Using Conformance Checking (All Best Matching)", requiredParameterLabels = { 0, 1 })
	public Object[] replayLog(UIPluginContext context, PetrinetGraph net,  XLog originalLog) {
		...
		Marking m_final = (Marking)PNLogMatchInstancesReplayer.constructFinalMarking(context, net)[1];
		
		ReplayParams par = PNLogMatchInstancesReplayer.getReplayerParameters(context, net, log, mapping.getEventClassifier(), m_initial, m_final);
		for (XEventClass e : par.mapEvClass2Cost.keySet()) {
			par.mapEvClass2Cost.put(e, 100);
		}
		PNMatchInstancesRepResult res = PNLogMatchInstancesReplayer.callReplayer(context, net, log, mapping, par, true);
