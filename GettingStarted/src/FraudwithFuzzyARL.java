...
public class FraudwithFuzzyARL {
	...
	public JPanel FraudTabel(final UIPluginContext context)
	{
		...
		InteractionResult result1 = context.showConfiguration("Percent Table", new Fuzzy().PercenTabel());
		if (result1.equals(InteractionResult.CANCEL)) {
			context.getFutureResult(0).cancel(true);
		}
		
		InteractionResult result2 = context.showConfiguration("Fuzzy Table", new Fuzzy().FuzzyTabel());
		if (result2.equals(InteractionResult.CANCEL)) {
			context.getFutureResult(0).cancel(true);
		}
		
		
		InteractionResult result3 = context.showConfiguration("ARL Parameter", new ARLParameter().ARLParam());
